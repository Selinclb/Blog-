...
def index(request):
    posts = Pposts = Post.objects.filter(status='published').order_by('-created_at')
    site_config = Settings.objects.first()  # eklendi
    return render(request, 'blog/index.html', {'posts': posts, 'site_config': site_config})
...
def post_list(request):
    ...
    site_config = Settings.objects.first()  # eklendi
    return render(request, 'blog/post_list.html', {'posts': posts, 'tags':tags, 'site_config': site_config})


def post_detail(request, slug):
    post = get_object_or_404(Post, slug=slug)
    site_config = Settings.objects.first()  # eklendi
    return render(request, 'blog/post_detail.html', {'post': post, 'site_config': site_config})
